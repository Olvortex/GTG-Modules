...
class TempMailMod(loader.Module):
    ...
    async def cemailcmd(self, message):
        ...
        for i in range(len(all_letters)):
            ...
            letter = requests.get(f'https://www.1secmail.com/api/v1/?action=readMessage&login={letter_demo}&domain={letter_domain}&id={letter_id}')

            try: topic = letter[i]["subject"] except: topic = 'None'
            try: email_message = letter[i]["textBody"] except: email_message = 'None'
