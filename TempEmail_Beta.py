...
class TempMailMod(loader.Module):
    ...
    async def chat_check(self, group_id, message):
        check = await self.client.get_entity(group_id)
    ...
    async def get_owner(self):
        return str((await self.client.get_me()).id)
    ...
    async def nemailcmd(self, message):
        ...
        await self.set_db(key = 'email', value = str(login))
        await self.set_db(key = 'emaildomain', value = domain)

        # Resetting the total number of received letters
        # self.db.set(owner = str((await self.client.get_me()).id), key = 'letters', value = 0)
        await self.set_db(key = 'letters', value = 0)
    ...
    async def emailcmd(self, message):
        ...
        login = await self.get_db(key = 'email')
        domain = await self.get_db(key = 'emaildomain')
    ...
    async def cemailcmd(self, message):
        ...
        # login = self.db.get(owner = str((await self.client.get_me()).id), key = 'email')
        # domain = self.db.get(owner = str((await self.client.get_me()).id), key = 'emaildomain')
        # letters = self.db.get(owner = str((await self.client.get_me()).id), key = 'letters')

        group_id = int(await self.get_db(key = 'emailchat'))
        login = await self.get_db(key = 'email')
        domain = await self.get_db(key = 'emaildomain')
        letters = int(await self.get_db(key = 'letters'))
        ...
        if str(all_letters) == '[]' or letters_now == letters:
            ...
            letters = await self.get_db(key = 'letters')
...
✏️ Message:\n{letter["textBody"]}')
        ...
        await self.set_db(key = 'letters', value = letters_now)
    ...
    async def emailchatcmd(self, message):
        ...
        try:
            return await self.client.send_message(group_id, '1')
            ...
            await self.set_db(key = 'emailchat', value = f'-100{group_id}')

            return await message.edit('⚠️ A new group for receiving letters has been created. Send the command again')